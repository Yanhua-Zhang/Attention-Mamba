...
from .utils_metrics_all import calculate_all_metrics_MultiClass, calculate_all_metrics_BinarySeg
...
# add:         
# GT_tensor_binarized = torch.where(GT_tensor >= 0.5, 1.0, 0.0)  # this operation is necessary, as we use bilinear for GT resize
# add calculation of avg f1, Acc, Recall, Spe, Prec of all cases of all classes
...
def calculate_from_folder_ISIC(logger, softmax_folder, GT_folder, path_save_folder, num_thresholds, name_list, num_classes):
    ...
    all_Acc_array = np.zeros((len(name_list), num_classes-1)) 
    all_Recall_array = np.zeros((len(name_list), num_classes-1))
    all_Spe_array = np.zeros((len(name_list), num_classes-1))
    all_Prec_array = np.zeros((len(name_list), num_classes-1))
    all_f1_array = np.zeros((len(name_list), num_classes-1))
    ...
    for name in name_list:
        ...
        GT_tensor = torch.from_numpy(GT_np).cuda()    # y, x ---> long ---> GPU
        ...
        GT_tensor_binarized = torch.where(GT_tensor >= 0.5, 1.0, 0.0)

        logger.info('shape of GT_binarized: ' + str(GT_tensor_binarized.size()))
        logger.info('Unique of GT_binarized: ' + str(torch.unique(GT_tensor_binarized)))
        ...
        GT_tensor_binarized = GT_tensor_binarized.unsqueeze(0).unsqueeze(0)  # y, x ---> 1, 1, y, x
        ...
        onehot_GT = fun_onehot_label(GT_tensor_binarized)    # 1, 1, y, x ---> C, 1, y, x
        ...
        results_BinarySeg = calculate_all_metrics_BinarySeg(label_pred, GT_tensor_binarized)  
        logger.info('results_BinarySeg: ' + str(results_BinarySeg))
        ...
        all_Acc_array[num_row,0:] = results_BinarySeg['Acc'] 
        all_Recall_array[num_row,0:] = results_BinarySeg['Sen(Recall)'] 
        all_Spe_array[num_row,0:] = results_BinarySeg['Spe']
        all_Prec_array[num_row,0:] = results_BinarySeg['Prec']
        all_f1_array[num_row,0:] = results_BinarySeg['f1(dsc)']
        ...
        for i in range(num_classes-1):
            case_dict['Acc_class_'+str(i+1)] = np.round(np.float64(results_BinarySeg['Acc'][i]), 6)
        for i in range(num_classes-1):
            case_dict['Sen(Recall)_class_'+str(i+1)] = np.round(np.float64(results_BinarySeg['Sen(Recall)'][i]), 6)
        for i in range(num_classes-1):
            case_dict['Spe_class_'+str(i+1)] = np.round(np.float64(results_BinarySeg['Spe'][i]), 6)
        for i in range(num_classes-1):
            case_dict['Prec_class_'+str(i+1)] = np.round(np.float64(results_BinarySeg['Prec'][i]), 6)
        for i in range(num_classes-1):
            case_dict['f1(dsc)_class_'+str(i+1)] = np.round(np.float64(results_BinarySeg['f1(dsc)'][i]), 6)
        ...
        case_dict['mean_Acc'] = np.round(np.float64(results_BinarySeg['mean_Acc']), 6)
        case_dict['mean_Sen(Recall)'] = np.round(np.float64(results_BinarySeg['mean_Sen(Recall)']), 6)
        case_dict['mean_Spe'] = np.round(np.float64(results_BinarySeg['mean_Spe']), 6)
        case_dict['mean_Prec'] = np.round(np.float64(results_BinarySeg['mean_Prec']), 6)
        case_dict['mean_f1(dsc)'] = np.round(np.float64(results_BinarySeg['mean_f1(dsc)']), 6)
    ...
    # calculation of avg f1, Acc, Recall, Spe, Prec of all cases of all classes
    ...
    logger.info('the Acc of all cases of all classes: ' + str(all_Acc_array))
    logger.info('the Recall of all cases of all classes: ' + str(all_Recall_array))
    logger.info('the Spe of all cases of all classes: ' + str(all_Spe_array))
    logger.info('the Prec of all cases of all classes: ' + str(all_Prec_array))
    logger.info('the f1 of all cases of all classes: ' + str(all_f1_array))
    ...
    avg_each_class_all_Acc_array = np.nanmean(all_Acc_array, axis=0)
    avg_each_class_all_Recall_array = np.nanmean(all_Recall_array, axis=0)
    avg_each_class_all_Spe_array = np.nanmean(all_Spe_array, axis=0)
    avg_each_class_all_Prec_array = np.nanmean(all_Prec_array, axis=0)
    avg_each_class_all_f1_array = np.nanmean(all_f1_array, axis=0)
    ...
    logger.info('the avg Acc of each cases of all classes: ' + str(avg_each_class_all_Acc_array))
    logger.info('the avg Recall of each cases of all classes: ' + str(avg_each_class_all_Recall_array))
    logger.info('the avg Spe of each cases of all classes: ' + str(avg_each_class_all_Spe_array))
    logger.info('the avg Prec of each cases of all classes: ' + str(avg_each_class_all_Prec_array))
    logger.info('the avg f1 of each cases of all classes: ' + str(avg_each_class_all_f1_array))
    ...
    avg_all_Acc_array = np.mean(avg_each_class_all_Acc_array)
    avg_all_Recall_array = np.mean(avg_each_class_all_Recall_array)
    avg_all_Spe_array = np.mean(avg_each_class_all_Spe_array)
    avg_all_Prec_array = np.mean(avg_each_class_all_Prec_array)
    avg_all_f1_array = np.mean(avg_each_class_all_f1_array)
    ...
    logger.info('the avg Acc of all cases of all classes: ' + str(avg_all_Acc_array))
    logger.info('the avg Recall of all cases of all classes: ' + str(avg_all_Recall_array))
    logger.info('the avg Spe of all cases of all classes: ' + str(avg_all_Spe_array))
    logger.info('the avg Prec of all cases of all classes: ' + str(avg_all_Prec_array))
    logger.info('the avg f1 of all cases of all classes: ' + str(avg_all_f1_array))
    ...
    for i in range(num_classes-1):
        all_case_dict['Acc_class_'+str(i+1)] = np.round(np.float64(avg_each_class_all_Acc_array[i]), 6)
    for i in range(num_classes-1):
        all_case_dict['Recall_class_'+str(i+1)] = np.round(np.float64(avg_each_class_all_Recall_array[i]), 6)
    for i in range(num_classes-1):
        all_case_dict['Spe_class_'+str(i+1)] = np.round(np.float64(avg_each_class_all_Spe_array[i]), 6)
    for i in range(num_classes-1):
        all_case_dict['Prec_class_'+str(i+1)] = np.round(np.float64(avg_each_class_all_Prec_array[i]), 6)
    for i in range(num_classes-1):
        all_case_dict['f1_class_'+str(i+1)] = np.round(np.float64(avg_each_class_all_f1_array[i]), 6)
    ...
    all_case_dict['mean_Acc'] = np.round(np.float64(avg_all_Acc_array), 6)
    all_case_dict['mean_Recall'] = np.round(np.float64(avg_all_Recall_array), 6)
    all_case_dict['mean_Spe'] = np.round(np.float64(avg_all_Spe_array), 6)
    all_case_dict['mean_Prec'] = np.round(np.float64(avg_all_Prec_array), 6)
    all_case_dict['mean_f1'] = np.round(np.float64(avg_all_f1_array), 6)
