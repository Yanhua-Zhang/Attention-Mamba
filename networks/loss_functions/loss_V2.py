...
# modified from: https://github.com/yhygao/CBIM-Medical-Image-Segmentation


# -----------------------------------------------------------------------------
# Cross Entropy Loss + Dice loss

class CE_and_Dice_loss_V2(nn.Module):
    def __init__(self, class_weights=None):
        super(CE_and_Dice_loss_V2, self).__init__()

        self.ce_loss = nn.CrossEntropyLoss(weight=torch.tensor(class_weights).cuda().float())
        self.dice_loss = DiceLoss()

    def forward(self, input, label):
        
        # for CE_and_Dice_loss, label: b, 1, h, w ---> b, h, w
        loss_ce = self.ce_loss(input, label.long().squeeze(1))
        
        if len(input.shape) != len(label.shape):
            # sometimes, the lable need do: z, y, x ---> z, 1, y, x
            loss_dice = self.dice_loss(input, label.long().unsqueeze(1))

        else:
            loss_dice = self.dice_loss(input, label.long())
